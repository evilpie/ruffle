...
impl UrlBlockRule {
    pub fn new(fetch_reasons: Vec<FetchReason>, host: impl ToString) -> Self {
        Self {
            fetch_reasons: fetch_reasons.into_iter().collect(),
            host: host.to_string(),
        }
    ...
    pub fn builtin_rules() -> Self {
        Self {
            rule_sets: vec![
                ...
                // The Mochiads currently don't work anymore and the moachiads.com domain is up for sale. There is a real concern about a malicious party buying it.
                RuleSet {
                    name: "mochiads".to_string(),
                    domain_rewrite_rules: vec![],
                    domain_block_rules: vec![
                        UrlBlockRule::new(FetchReason::all(), "mochiads.com"),
                        UrlBlockRule::new(FetchReason::all(), "*.mochiads.com"),
                    ],
                },
            ],
...
mod tests {
    ...
    fn test_domain_matches() {
        ...
        assert!(!domain_matches("com", "foo.example.com"));
        assert!(!domain_matches("example.com", "foo.example.com"));
        assert!(!domain_matches("*.example.com", "exmaple.com"));
